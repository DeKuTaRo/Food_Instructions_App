...
import { Typography, Paper, Grid, CardMedia,Button,Dialog,DialogTitle,DialogContent,FormControl,RadioGroup,FormControlLabel,Radio,DialogActions } from "@mui/material";
    ...
    const getUserData = async () => {
      try {
        const res = await axios.get(
          `http://localhost:8004/order/id/${orderId}`,
          {
            headers: {
    ...
    const fetchRecipeDetails = async () => {
      try {
        const response = await axios.get(
          `${encodeURI(orderDetails.productLink)}`
        );
 ...
 const handleConfirmPayment = () => {
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      case "NotPayment":
        return (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "end", width: "100%",flexDirection:"column" }}>
            <div>
      ...
      >
        ...
        <Paper elevation={3} style={{ marginLeft: "0px", width: "100%" }}>
          ...
          <Grid
            container
            spacing={2}
            style={{ backgroundColor: "white", width: "100%", margin: "0px" }}
          >
