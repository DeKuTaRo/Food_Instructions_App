...
import inforTopic from "./Topic2Data";
...
useEffect(() => {
    const fetchRecipes = async () => {
      try {
        const mealType = label; // Replace with the desired dishType
        const appId = process.env.REACT_APP_APP_ID_RECIPE;
        const appKey = process.env.REACT_APP_APP_KEY_RECIPE;
        const response = await axios.get(
          `https://api.edamam.com/api/recipes/v2?type=public&app_id=${appId}&app_key=${appKey}&mealType=${mealType}`
        );
  ...
  return (
    <motion.div
      ...
      <Grid container spacing={2}>
        {relatedRecipes.slice(0, 16).map((recipe) => (
